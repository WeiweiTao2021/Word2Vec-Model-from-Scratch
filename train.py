...
class Trainer:
    ...
    """
    # Add negative word for negative sampling
    """
    def training_step(self, center_word, context_word, negative_word):
        loss =  self.model(center_word, context_word, negative_word)
        return loss
    ...
    def train(self, dataset, max_training_steps, ckpt_steps, validation_words, device="cpu"):
        ...
        for curr_step in t:
            optim.zero_grad()
            center_word, context_word, negative_word = dataset.generate_batch()
            loss = self.training_step(center_word.to(device), context_word.to(device), negative_word.to(device))
            loss.backward()
            optim.step()
