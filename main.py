...
"""
# Add global var neg_sample_size for NEG loss
"""
...
def parse_args():
    ...
    parser.add_argument("--neg_sample_size", help="Negative sample size", type=int, default=2)
...
if __name__ == "__main__":
    ...
    if args.loss_model == "nll":
        args.neg_sample_size = 0
    ...
    # Calculate the probability of unigrams: c^{3/4}/sum(c^{3/4})
    count_dic = dict(count)
    sample_prob = [pow(count_dic[vocab_id_to_token[i]], 3/4) for i in sorted(list(vocab_token_to_id.values()))]
    total_counts = sum(sample_prob)
    unigram_prob = [c/total_counts for c in sample_prob]

    #generate data batches
    data_index = 0
    dataset = Dataset(data, unigram_prob, batch_size=args.batch_size, num_skips=args.num_skips, skip_window=args.skip_window, neg_sample_size = args.neg_sample_size)
    center, context, negative_context = dataset.generate_batch()
    for i in range(8):
    ...
    model = WordVec(V=args.vocab_size, embedding_dim=embedding_size, loss_func=args.loss_model)
    ...
    model_filepath = os.path.join(model_path, 'word2vec_%s_skip_%d_batch_%d_embsize_%d_nsample_%d.model'%(args.loss_model, args.num_skips, args.batch_size, args.embedding_size, args.neg_sample_size))
